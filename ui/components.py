...
.wb-3s { margin: .2rem 0 .8rem 0; }
.wb-3s-hd { font-family: var(--wb-mono); font-size:.68rem; letter-spacing:.1em; color: var(--wb-muted);
            margin-top:10px; }
.wb-3s-txt { font-size:.86rem; color: var(--wb-text); line-height:1.4; margin: 2px 0 0 0; }
.wb-gallery-caption { font-family: var(--wb-mono); font-size:.72rem; color: var(--wb-muted); margin-top:4px; }
.wb-project-card { border:1px solid var(--wb-border); border-radius:6px; padding:14px 16px; background:var(--wb-panel);
                    margin-bottom:10px; }
.wb-project-card .ttl { font-weight:700; font-size:1.05rem; color:#e6ebf0; margin:2px 0 4px 0; }
</style>
"""
...
_GUIDED_BADGE_CLASS = {
    gs.BADGE_AVAILABLE: "m-qualified",
    gs.BADGE_FOUNDATION: "m-foundation",
    gs.BADGE_PARTIAL: "m-partial",
    gs.BADGE_COMING_SOON: "m-construction",
}


def mode_toggle() -> str:
    """The Guided View / Engineering View switch. Default 'guided'. Returns the active mode and
    keeps it in `st.session_state['ui_mode']`."""

    st.session_state.setdefault("ui_mode", "guided")
    left, right, _rest = st.columns([1, 1, 6])
    if left.button(
        "Guided View",
        type="primary" if st.session_state.ui_mode == "guided" else "secondary",
        use_container_width=True,
    ):
        st.session_state.ui_mode = "guided"
    if right.button(
        "Engineering",
        type="primary" if st.session_state.ui_mode == "engineering" else "secondary",
        use_container_width=True,
    ):
        st.session_state.ui_mode = "engineering"
    return st.session_state.ui_mode


def guided_badge(badge: str | None) -> str:
    """Compact AVAILABLE/FOUNDATION/PARTIAL/COMING SOON chip HTML, or '' when a guided stage
    has no tied hazard capability (caller shows an evidence_chip instead)."""

    if badge is None:
        return ""
    return chip(badge, _GUIDED_BADGE_CLASS.get(badge, "m-construction"))


def three_sentence_block(
    what_you_are_looking_at: str, what_it_means: str, what_it_does_not_mean: str
) -> None:
    st.markdown(
        f"""<div class="wb-3s">
<div class="wb-3s-hd">WHAT YOU ARE LOOKING AT</div>
<div class="wb-3s-txt">{_esc(what_you_are_looking_at)}</div>
<div class="wb-3s-hd">WHAT IT MEANS</div>
<div class="wb-3s-txt">{_esc(what_it_means)}</div>
<div class="wb-3s-hd">WHAT IT DOES NOT MEAN</div>
<div class="wb-3s-txt">{_esc(what_it_does_not_mean)}</div>
</div>""",
        unsafe_allow_html=True,
    )
...
def project_card(
    title: str, description: str, *, image_path: str | None = None, badge: str = "REAL SOURCE"
) -> bool:
    """One large Guided-View landing card. Returns True the run its 'Explore' button is
    clicked."""
    ...
    with st.container(border=True):
        ...
        st.markdown(
            f'<div class="wb-project-card"><div class="ttl">{_esc(title)}</div>',
            unsafe_allow_html=True,
        )
        st.caption(description)
        st.markdown(f"{chip(badge, 'e-real')}</div>", unsafe_allow_html=True)
        return st.button(f"Explore {title}", key=f"explore-{title}", use_container_width=True)


def stage_nav(items: Sequence[tuple[str, str, str | None]], active_index: int, *, key: str) -> int:
    """`items`: (number_label, stage_label, guided_badge_or_None) rows, in story order. Renders
    a numbered vertical stage list; returns the (possibly newly clicked) active index."""

    clicked = active_index
    for i, (number, label, badge) in enumerate(items):
        ...
        if st.button(
            ...
            clicked = i
        if badge:
            st.markdown(guided_badge(badge), unsafe_allow_html=True)
    return clicked


def map_gallery(
    items: list[tuple[str, Any]], *, key: str, technical_paths: Sequence[str] = ()
) -> None:
    """`items`: (label, figure_or_image_path) pairs, at least one. Shows one large active
    image/figure, with a thumbnail selector and Prev/Next when there is more than one. The
    active index is session-state-keyed by `key` so switching stages resets the selector.
    Filenames are hidden unless `technical_paths` is given and 'Show technical details' is
    opened."""

    if not items:
        mono("No visual output is present locally for this step.")
        return

    idx_key = f"gallery_idx_{key}"
    st.session_state.setdefault(idx_key, 0)
    st.session_state[idx_key] = min(st.session_state[idx_key], len(items) - 1)
    active_idx = st.session_state[idx_key]
    label, active = items[active_idx]

    if hasattr(active, "savefig"):
        st.pyplot(active, use_container_width=True)
        import matplotlib.pyplot as plt

        plt.close(active)
    else:
        st.image(str(active), use_container_width=True)
    st.markdown(f'<div class="wb-gallery-caption">{_esc(label)}</div>', unsafe_allow_html=True)

    if len(items) > 1:
        cols = st.columns(len(items) + 2)
        if cols[0].button("< Prev", key=f"{key}-prev"):
            st.session_state[idx_key] = (active_idx - 1) % len(items)
            st.rerun()
        for i, (item_label, _thumb) in enumerate(items):
            if cols[i + 1].button(
                item_label,
                key=f"{key}-thumb-{i}",
                ...
                st.rerun()
        if cols[-1].button("Next >", key=f"{key}-next"):
            st.session_state[idx_key] = (active_idx + 1) % len(items)
            st.rerun()

    if technical_paths:
        with st.expander("Show source files", expanded=False):
            for p in technical_paths:
                mono(p)


def layer_control(layers: Sequence[tuple[str, bool, bool]], *, key: str) -> dict[str, bool]:
    """`layers`: (label, available, default_on) rows. An unavailable layer renders as a
    disabled checkbox -- it can never be turned on, not merely styled to look inactive. Returns
    {label: is_checked}."""

    result: dict[str, bool] = {}
    for label, available, default_on in layers:
        widget_key = f"layer_{key}_{label}"
        requested = st.session_state.get(widget_key)
        checked = st.checkbox(
            label if available else f"{label} (not available)",
            value=mv.resolve_layer_state(available, default_on, requested),
            disabled=not available,
            key=widget_key,
        )
        result[label] = mv.resolve_layer_state(available, default_on, checked)
    return result


def cpt_profile_widget(test_ids: Sequence[str], *, key: str) -> None:
    if not test_ids:
        mono("No CPT tests are present locally.")
        return
    test_id = st.selectbox("CPT test", list(test_ids), key=f"{key}-test-id")
    fig, meta = mv.render_cpt_profile(test_id)
    if fig is None:
        mono(f"No measured channels available for {test_id}.")
        return
    st.pyplot(fig, use_container_width=True)
    import matplotlib.pyplot as plt

    plt.close(fig)
    if meta["unavailable_channels"]:
        names = ", ".join(c.split("_")[0] for c in meta["unavailable_channels"])
        st.markdown(
            chip(f"Unavailable for this source: {names}", "e-missing"), unsafe_allow_html=True
        )
    st.caption("Measured channels only. No CRR, CSR or liquefaction calculations.")
