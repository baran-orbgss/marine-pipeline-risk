...
def render_engineering_view() -> None:
    with st.sidebar:
        ...
        project_id = st.selectbox(
            ...
            key="engineering_project_id",
        )
        page = st.radio("Page", PAGES, label_visibility="collapsed", key="engineering_page")
        st.divider()
    ...
    def page_overview() -> None:
        ...
        st.caption(
            "Maturity legend: "
            + " ".join(
                f"{m} = {registry.MATURITY_MEANING[m]}" for m in registry.MATURITY_VOCABULARY
            )
        )
    ...
    def page_capability_explorer() -> None:
        ...
        cap_id = st.selectbox(
            ...
            format_func=lambda cid: (
                f"{registry.hazard_by_id(cid).hazard_number:02d}  "
                f"{registry.hazard_by_id(cid).title}"
            ),
        )
        ...
        with right:
            ...
            st.markdown("**CLI**")
            c.mono(
                "\n".join(f"uv run marine-engine {cmd} <config>" for cmd in cap.cli_commands)
                or "none"
            )
            st.markdown("**Tickets**")
    ...
    def page_test_lab() -> None:
        ...
        kind = st.radio(
            "Registry", ("Hazard capabilities", "Supporting capabilities"), horizontal=True
        )
        if kind == "Hazard capabilities":
            items = [
                (cap.id, f"{cap.hazard_number:02d} {cap.title}", cap.tests)
        ...
        with st.expander("Advanced", expanded=False):
            st.warning(
                "Full offline suite: ~1850 tests, several minutes, high CPU and memory. Not "
                "needed for normal workbench use. Live/network tests stay excluded."
            )
    ...
    def _inspect_entry(entry: oi.OutputEntry) -> None:
        ...
        if entry.kind == "json":
            ...
            if entry.is_large:
                st.warning(
                    f"Large file ({entry.byte_size:,} bytes): only the first batch is read on "
                    "request."
                )
            ...
            if st.button("Render display preview (decimated)", key=f"prev-{entry.relative_path}"):
                ...
                ax.set_title(
                    f"{entry.relative_path}\n{preview['source_shape']} -> "
                    f"{preview['preview_shape']} (x{preview['decimation_factor']:.1f} decimation)",
                    fontsize=8,
                )
                ax.set_axis_off()
            ...
            if st.checkbox(
                "Render HTML report (sandboxed iframe)", key=f"html-{entry.relative_path}"
            ):
    ...
    def page_data_outputs() -> None:
        ...
        for rel in project.readiness_files:
            ...
            st.json(
                summary or {"note": "no top-level status field written by the engine"},
                expanded=False,
            )
        ...
        if not entries:
            c.mono(
                oi.NOT_PRESENT_LOCALLY + " -- no processed/interim outputs exist for this project."
            )
        else:
        ...
        for cap in registry.capabilities_for_project(project.id):
            with st.expander(
                f"{cap.hazard_number:02d} {cap.title} -- {cap.maturity}", expanded=False
            ):
                if cap.output_patterns:
    ...
    def page_roadmap() -> None:
        ...
        st.caption(
            "No ETA is stated for any capability. Order is the canonical hazard-family order."
        )
