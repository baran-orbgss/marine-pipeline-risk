...
maturity registry and a map-first guided walkthrough.
...
* `guided_story`       -- Guided View story data (per-project stage sequences, plain-language
  text, visual candidates). Pure Python; no Streamlit import.
...
* `map_view`           -- bounded, display-only map/chart rendering for Guided View (returns
  matplotlib Figures; reprojects copies, never the source). Pure Python; no Streamlit import.
