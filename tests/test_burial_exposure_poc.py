...
def test_A_burial_reference_must_be_resolved_before_exposure_inference():
    # a strongly negative value (which might "look like" exposure under a naive convention)
    # must NOT become SOURCE_INTERPRETED_EXPOSED just because the reference is unresolved.
...
def test_F_measured_state_and_source_interpreted_exposure_remain_distinct():
    ...
    result = profile.build_canonical_burial_profile(
        ...
        burial_sign_convention="positive-down",
        survey_epoch="2020",
        measurement_method=None,
        exposure_flag_column="is_exposed",
    )
    states = result.sort_values("chainage_m")["current_burial_state"].tolist()
...
def test_G_trench_scar_is_not_automatically_exposure():
    ...
    # descriptor) must never resolve to SOURCE_INTERPRETED_EXPOSED, regardless of its measured
    # value.
    state = profile.classify_current_burial_state(
        0.0,
        burial_reference_type=semantics.TOP_OF_ASSET_BURIAL,
...
def test_L2_no_measured_burial_means_insufficient_input():
...
def test_P2_no_dataset_name_string_in_generic_engine_logic():
    ...
    # identity at all.
    non_map_modules = (contract, exposure_screening, margin, profile, readiness, route, semantics)
