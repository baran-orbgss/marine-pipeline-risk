...
- **MAR-026A (project CRS integrity & registration fail-safe repair,
  ...
  ticket has started.
- **MAR-027 (canonical project route-reference model & cross-asset
  linkage POC, `src/marine_engine/project/route_reference.py`,
  `src/marine_engine/project/model.py`, CLI command `build-project-model`).**
  The bridge between MAR-026/026A generic registration + readiness and
  future independent route-referenced geohazard engines -- no new
  geohazard model, no hazard fusion, no route-suitability/project/risk
  score, no temporal-equivalence rule, no CRS or route inference. The
  `ProjectManifest` schema is extended narrowly (still `extra="forbid"` on
  every new model): an OPTIONAL project-level `route_reference: {interval_m}`
  (finite, strictly > 0, metres -- an indexing resolution, never a survey
  accuracy; when the section is absent no default spacing is invented) and
  an OPTIONAL per-asset `route_relationship: {route_asset_id,
  relationship_type: ROUTE_REFERENCED, linear_reference?: {basis:
  CANONICAL_ROUTE_FROM_GEOMETRY_START, units: m}}` whose target must exist,
  must be a `PIPELINE_ROUTE`, and may not be the asset itself (each
  rejected at manifest validation; the vocabularies live in
  `project.categories` and are deliberately narrow). `primary_route_asset_id`
  is now operational (`project.route_reference.resolve_primary_route`): the
  declared route drives the model only when it registered, is effectively
  `READY`/`READY_WITH_LIMITATIONS`, carries no declared-vs-observed CRS
  conflict, the project `working_crs` is valid/projected/metric, and a
  canonical route was produced -- otherwise a controlled
  `PRIMARY_ROUTE_UNUSABLE` finding, never a crash and never a fallback to
  another route; an absent declaration is `PRIMARY_ROUTE_NOT_DECLARED`
  even when exactly one usable route exists (proven). When every
  prerequisite holds, a deterministic CANONICAL ROUTE-REFERENCE GRID is
  built (`project_route_reference.gpkg`, layer `route_reference_points`,
  project working CRS): chainage 0 at the canonical route geometry start
  (`chainage_origin_basis = SOURCE_GEOMETRY_ORDER`, read from the accepted
  canonical route -- never from a pipe name or filename), regular stations
  plus the exact terminus, strictly increasing, no duplicates, unrounded
  `chainage_m` with a display-only `kp_label` -- generated by the accepted
  MAR-004 primitives `preprocessing.chainage.compute_chainage_stations` /
  `format_kp_label` through a project-specific wrapper, never
  re-implemented (proven structurally: no `interpolate(` in the wrapper).
  Grid points are deliberately never called "supports": the grid is a
  linear/spatial indexing framework for downstream alignment and carries an
  explicit no-physical-support disclaimer. Cross-asset linkage is a
  SEPARATE axis (`project.model`: `LINKED` / `LINKED_WITH_LIMITATIONS` /
  `UNRESOLVED` / `NOT_APPLICABLE`) that never overloads
  `readiness_status_intrinsic`/`readiness_status_effective` (the model layer
  never imports or reads any readiness module or `readiness_result`): an
  asset without a manifest relationship is `NOT_APPLICABLE` (proven never
  inferred from filename, shared directory, matching CRS, or category, and
  never marked defective for it); a registration failure, an unresolved
  declared-vs-observed conflict, or an unusable referenced route is
  `UNRESOLVED`; linkage is evaluated against the DECLARED route, which need
  not be the primary route. A burial profile's `chainage_or_kp_column`
  alone is explicitly insufficient -- numeric range correlation
  (`source_chainage_min/max_m`, `records_within/outside_route_chainage_range`,
  `chainage_range_overlap_m`, `chainage_range_overlap_fraction`,
  `duplicate_numeric_chainage_count`) is computed only under an explicit
  `linear_reference` declaration (units `m` only; `units_declared`, which
  describes the measured value, is never used for chainage), out-of-route
  and duplicate values are reported (never clipped or collapsed), the field
  is deliberately named range-overlap rather than coverage, and the
  accepted MAR-024 intrinsic `coverage_fraction` limitation is left
  untouched. Raster linkage exposes only `raster_extent_intersects_route` /
  `raster_extent_route_overlap_length_m`, worded as observed-extent only --
  never a `bathymetry_coverage_fraction`. Declared survey epochs are
  reported side by side with no `TEMPORALLY_COMPATIBLE` claim. Outputs per
  project (`data/processed/<project_id>/project/`):
  `canonical_project_model.json` (identity, working CRS + findings,
  primary-route status, route-reference status/basis/length/station counts,
  per-asset evidence role, intrinsic/effective readiness, declared
  relationship, linkage status/findings/facts, explicit limitations -- no
  timestamps, no absolute paths, no aggregate project status, no score;
  proven byte-identical across two builds), `project_asset_linkage.parquet`
  (one row per manifest asset, list fields JSON-encoded),
  `project_model_validation.json`, and the GeoPackage ONLY when a grid was
  actually built (a stale grid from an earlier run of the same command is
  removed, proven). 69 new tests (`tests/test_project_route_reference.py`)
  cover every Section 28 proof point: interval validation (positive
  accepted; zero, negative, NaN, +/-inf rejected), relationship validation
  (nonexistent target, non-route target, self-reference, unknown type,
  unsupported linear-reference basis/units all rejected), primary-route
  semantics (absent -> no auto-selection, unusable -> controlled finding,
  never replaced by another usable route, non-route primary, CRS conflict,
  invalid/geographic working CRS -> no grid), grid invariants (exact 0
  start, exact terminus, strictly increasing, no duplicates, deterministic,
  non-divisible residual retained, exactly-divisible has no extra point,
  CRS == working CRS even for an ED50 source route, reversed source
  geometry reverses the grid, KP label display-only), evidence/readiness
  separation (evidence role, intrinsic, MAR-026A effective, and
  `readiness_result` all unchanged; `SOURCE_INTERPRETED`/`DERIVED` never
  re-labelled), burial semantics (column alone insufficient, explicit
  declaration gives deterministic facts, out-of-route reported not
  clipped, duplicates visible, range overlap not mislabelled coverage,
  unresolved reference/sign stay unresolved), raster extent (intersecting,
  non-intersecting, and an EPSG:4326-embedded raster placed via the
  declared no-conflict path), outputs (deterministic JSON, one parquet row
  per asset, GeoPackage only when prerequisites pass, no score/percent/
  `project_ready` key anywhere), and CLI wiring. Real cached-data runs,
  fully offline: PL854 -- the accepted MAR-002 canonical route
  (`data/processed/pl854/pipeline.gpkg`, EPSG:32631, one LineString,
  23,480.669 m) registered as primary route with the accepted MAR-006
  EMODnet baseline raster declared `ROUTE_REFERENCED`
  (`configs/project_manifests/pl854_route_reference.yaml`; both
  `survey_epoch`s deliberately unset because neither a registry retrieval
  date nor a compilation-product year is a survey epoch, so both assets
  honestly register `READY_WITH_LIMITATIONS`) -> `PRIMARY_ROUTE_AVAILABLE`,
  `ROUTE_REFERENCE_BUILT`, 941 stations (940 regular at 25 m + a 5.669 m
  terminal residual), first/last station coinciding with the route
  start/end, max station-to-route distance 4.6e-10 m, and the grid's
  `chainage_m`, station geometry, and KP labels byte-identical to the
  accepted MAR-004 `chainage_25m.gpkg` (941 stations, max abs diff 0.0);
  the raster links `LINKED` with `raster_extent_intersects_route=true` and
  an observed-extent-vs-route overlap of the full 23,480.669 m -- extent
  only, not valid-cell coverage (its delegated MAR-020 readiness reports
  `valid_cell_fraction=0.781`). Sheringham Shoal 2020: unchanged
  `observed_crs=EPSG:32631`, `conflicts=[]`, intrinsic and effective
  `READY`; Barrow 2016: unchanged `READY_WITH_LIMITATIONS` with the
  identical unresolved burial-reference limitation and the same 1
  duplicate-KP finding; both honestly report `PRIMARY_ROUTE_NOT_DECLARED` /
  `ROUTE_REFERENCE_NOT_BUILT` / `NOT_APPLICABLE` with no GeoPackage
  written, rather than being treated as failed projects, and no route was
  invented from Barrow's burial x/y or from Barrow's numeric `chainage_m`.
  `terrain/readiness.py`, `burial/readiness.py`, `project/registry.py`,
  `project/route_adapter.py`, `project/burial_adapter.py`,
  `project/bathymetry_adapter.py`, and `preprocessing/chainage.py` are
  untouched; `build-project-readiness` behaviour is unchanged (proven by
  test). Full offline suite 1497 passed (up from 1428), 3 skipped;
  repo-wide `ruff format`/`ruff check` clean. Observed but deliberately not
  repaired here (outside MAR-027 scope): the accepted MAR-026 bathymetry
  adapter emits a NumPy `RuntimeWarning` (`atol: nan`) when a raster's
  nodata value is NaN, as PL854's canonical raster's is -- the delegated
  readiness result itself is unaffected. Final answers: `DOES MAR-027
  CREATE A DETERMINISTIC CANONICAL ROUTE-REFERENCE GRID?` YES (PL854 real
  run); `CAN AN ASSET BECOME ROUTE-LINKED WITHOUT AN EXPLICIT MANIFEST
  RELATIONSHIP?` NO; `CAN A BURIAL PROFILE'S NUMERIC KP/CHAINAGE COLUMN BE
  ASSUMED TO MATCH CANONICAL PROJECT CHAINAGE WITHOUT EXPLICIT
  LINEAR-REFERENCE SEMANTICS?` NO; `DOES ROUTE LINKAGE MODIFY EVIDENCE ROLE
  OR INTRINSIC READINESS?` NO; `DOES MAR-027 CLAIM RASTER-BOUND
  INTERSECTION IS VALID-DATA COVERAGE?` NO; `DOES MAR-027 CREATE A
  UNIVERSAL PROJECT HAZARD/READINESS SCORE?` NO; `ARE MAR-020, MAR-024,
  MAR-026A SCIENTIFIC/READINESS SEMANTICS PRESERVED?` YES. Still future
  (explicitly not implemented): coordinate-based burial projection onto
  the route (`project_point_to_route`), valid-cell bathymetry route
